...
@jsonrpc.method('set_output(pin=int, new_state=int)', validate=True, authenticated=check_auth)
def set_output(pin, new_state):
    GPIO.output(pin, new_state)
    return {u'status': {'output': {'pins': {pin: new_state}}}}


@jsonrpc.method('get_status(pin=int)', validate=True, authenticated=check_auth)
def get_status(pin):
